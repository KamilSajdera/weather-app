...
export async function fetchWeather(apiData: ApiFetch): Promise<object> {
  ...
  const endpointURL = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&hourly=relativehumidity_2m,pressure_msl,visibility&daily=weathercode,precipitation_probability_max,temperature_2m_max,temperature_2m_min,sunrise,sunset,uv_index_max,rain_sum,snowfall_sum&current_weather=true&timezone=auto&temperature_unit=${tempUnit}`;
