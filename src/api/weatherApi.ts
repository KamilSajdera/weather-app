...
export async function fetchWeather(apiData: ApiFetch): Promise<object> {
  ...
  const endpointURL = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&hourly=pressure_msl,visibility,temperature&daily=weathercode,precipitation_probability_max,temperature_2m_max,temperature_2m_min,sunrise,sunset,uv_index_max,rain_sum,snowfall_sum&current=relative_humidity_2m,temperature_2m,rain,surface_pressure,weather_code,wind_speed_10m&timezone=auto&temperature_unit=${tempUnit}`;
